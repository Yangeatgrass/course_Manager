...
(function ($) {
    $.extend({
        ...
        modal: {
            ...
            // 弹出层指定宽度
            NotTopOpen: function (title, url, width, height, callback) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            NoQropen: function (title, url, width, height, callback) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
        ...
        operate: {
            ...
            addBigWidth: function(id) {
                table.set();
                $.modal.open("添加" + table.options.modalName, $.operate.addUrl(id),1200,600);
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
                table.set();
            ...
            editBigWight: function(id) {
                ...
                if ($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.updateUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.open("修改" + table.options.modalName, url,1200,600);
                } else {
                    $.modal.open("修改" + table.options.modalName, $.operate.editUrl(id),1200,600);
                }
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
